...
import { CreateDetailKategoriData } from "../../lib/api/detailkategori";

const detailKategoriFormSchema = z.object({
...
type DetailKategoriFormSchema = z.infer<typeof detailKategoriFormSchema>;

interface CreateDetailKategoriModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (data: CreateDetailKategoriData) => Promise<void>;
  subkategoriId: string;
  subkategoriName?: string;
  kategoriName?: string;
}

export default function CreateDetailKategoriModal({
  isOpen,
  onClose,
  onSubmit,
  subkategoriId,
  subkategoriName,
  kategoriName,
}: CreateDetailKategoriModalProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);

  const form = useForm<DetailKategoriFormSchema>({
    resolver: zodResolver(detailKategoriFormSchema),
    defaultValues: {
      kode_detail_kategori: "",
      nama_detail_kategori: "",
      deskripsi: "",
      status: "aktif",
      subkategori_aset_id: parseInt(subkategoriId),
    },
  });

  const handleSubmit = async (values: DetailKategoriFormSchema) => {
    try {
